<template xmlns="http://www.w3.org/1999/html">
	...
	<div>
		...
		<Collapse @on-change="changePanel" style="text-align: left;" v-model="value2" accordion>
				...
				<div slot="content">
							...
							<div class="ivu-table-wrapper">
								<div v-model="value2" class="ivu-table-tab ivu-table-border" style="text-align: right;">
									<table cellspacing="0" cellpadding="0" border="0" style="width:100%;text-align: center">
										...
										<tr>
											<td v-for="val in costData" :key="val.pjTypeId">
					...
					<Modal title="修改管理费" width="820" v-model="modal1" class-name="vertical-center-modal" :mask-closable="false" :closable="false">
						<form name="roleForm" style="text-align: center;margin: 30px;">
							<Row>
								<Col span="3" v-for="val in costData" :key="val.pjTypeId" class="pjmenu">
								<span class="pjname">{{val.name}}类</span>
								<InputNumber style="max-width:60px;line-height:14px" :max="100" :min="0" v-model="val.rate"></InputNumber>
			...
			<Panel name="category" v-model="value2">
				...
				<div slot="content">
					<div>
						<Row>
							<Col span="2">
							<Button @click="modalOrganization = true" icon="android-add" type="primary" style="margin-bottom: 16px;">新添分类</Button>
							</Col>
							<Col span="24">
								...
								<div>
									...
									<div>
										...
										<form name="roleForm" style="text-align: center;margin-top: 30px;">
											<Row>
												<Col v-if="addClassify.name!='未分类'" span="3" v-for="val in addClassify.detail" :key="val.pjTypeId" class="pjmenu">
												<span class="pjname">{{val.name}}类</span>
												<InputNumber :max="100" style="max-width:60px;line-height:14px;"  :min="0" v-model="val.rate"></InputNumber>
												<!-- <input type="text" class="form-control-global" v-model='val.rate' /> -->

												<span class="form-control-icon">%</span>
												</Col>
												<Col v-if="addClassify.name=='未分类'" span="3" v-for="val in costData" :key="val.pjTypeId" class="pjmenu">
												<span class="pjname">{{val.name}}类</span>
												<InputNumber :max="100" style="max-width:60px;line-height:14px;" :min="0" v-model="val.rate"></InputNumber>
												<!-- <input type="text" class="form-control-global" v-model='val.rate' /> -->
												<span class="form-control-icon">%</span>
											...
											<div class="ivu-table-wrapper">
												<div class="ivu-table ivu-table ivu-table-border" style="text-align: right;">
													<table cellspacing="0" cellpadding="0" border="0" style="width:100%;text-align: center;">
														...
														<tr v-for="val in filteredXlc" :key="val.xlcId">
															<!--<td>-->
									...
									<div class="ivu-table-wrapper">
										<div v-model="value2" class="ivu-table ivu-table ivu-table-border" style="text-align: right;">
											<table v-model="value2" cellspacing="0" cellpadding="0" border="0" style="width:100% ;text-align: left;">
												...
												<tr>
													<th width="100">
														<div class="ivu-table-cell">
															<Checkbox
									            v-model="checked"
									            @on-change="allSel(checked,filteredUsers)"></Checkbox>
															<!-- <input type="checkbox" v-model="checked" v-on:change="allSel(checked,filteredUsers)"> -->
														</div>
													</th>
													<th>
														<div class="ivu-table-cell">修理厂名称</div>
												...
												<tr v-for="val in filteredUsers" :key="val.xlcId">
													<td>
														<Checkbox
														:value="isSelectedxlcpush(val.xlcId)"
														@on-change="chk(val.xlcId,val.categoryId)"></Checkbox>
								...
								<div v-model="value2" class="ivu-table-tab ivu-table-border" style="text-align: right;">
									<table v-model="value2" cellspacing="0" cellpadding="0" border="0" style="width:100%;text-align: center;">
										...
										<tr v-for="item in filterCategory">
											...
											<td v-if="item.categoryName!='未分类'" v-for="val in item.detail" :key="val.pjTypeId">
												{{val.rate>=0 ? val.rate+"%":'-'}}
											</td>
											<td v-if="item.categoryName=='未分类'" v-for="val in costData" :key="val.pjTypeId">
                        <span v-if='val.rate===""'>{{'-'}}</span>
												<span v-if="val.rate!=0">{{val.rate+"%"}}</span>
                        <span v-if="val.rate===0">{{val.rate+"%"}}</span>
											</td>
			...
			<Panel name="xlc">
				...
				<div class="example-case" slot="content">
					<div class="ivu-table-wrapper">
						<div v-model="value2" class="ivu-table-tab ivu-table-border" style="text-align: right;">
							<table cellspacing="0" cellpadding="0" border="0" style="width:100%;text-align: center">
								...
								<tr class="ivu-table-tab">
									<th width="30">
										<div>
											<Checkbox
											v-model="checked"
											@on-change="xlcSel(checked,filteredXlcData)"></Checkbox>
								...
								<tr class="ivu-table-tab" v-for="item in filteredXlcData" :key="item.xlcId">
									<td>
										<Checkbox
										:value="isxlcSelected(item.xlcId)"
										@on-change="bulkArr(item.xlcId)"></Checkbox>
									...
									<td v-for="item1 in item.detail" :key="item.pjTypeId">
					...
					<Modal class="highLevelModal" width="820" v-model="modal4" :mask-closable="false" :closable="false">
						<p slot="header">
							<span>{{modalXlcTitle}}</span>
						</p>
						<form name="roleForm" style="text-align: center;margin: 30px;">
							<Row>
								<Col span="3" v-for="val in xlcEditData" :key="val.pjTypeId"  class="pjmenu">
								<span class="pjname">{{val.name}}类</span>
								<InputNumber :max="100" style="max-width:60px;line-height:14px;" :min="0" v-model="val.rate"></InputNumber>
								<!-- <input type="text" class="form-control-global" v-model='val.rate' /> -->
								<span class="form-control-icon">%</span>
		...
		<Col span="3" offset="21">
			<Button type="primary" long @click="getUpdateCustomers(value2[0])">确认修改</Button>
		</Col>
		</Row>
	</div>
</template>
...
<script>
	import { fetchGet, fetchPostUrlencoded,fetchPostJson } from '../../../utils/requestHttp.js'
	...
	export default {
			...
			return {
			  commonInsId:'',
				...
				addClassify: {
					...
					xlcIds: [],
					detail: [
						{ name: 'A', "pjTypeId": 10, "rate": 0 },
						{ name: 'B', "pjTypeId": 11, "rate": 0 },
						{ name: 'C', "pjTypeId": 12, "rate": 0 },
						{ name: 'D', "pjTypeId": 13, "rate": 0 },
						{ name: 'E1', "pjTypeId": 14, "rate": 0 },
						{ name: 'E2', "pjTypeId": 15, "rate": 0 },
						{ name: 'E3', "pjTypeId": 16, "rate": 0 },
						{ name: 'E4', "pjTypeId": 17, "rate": 0 }
					],
          insId:''
				},
				verifynum: 0,
		...
		methods: {
			getCustomers() {
				var self = this;
				fetchGet('/nxp-crm/sys/setting/query?insId='+this.rowObj.id).then((res) => {
					if(res.flag != 0) {
            this.$Message.error(res.msg);
					} else {
						this.value2.push(res.msg);
						if(res.msg == 'all') {
							this.getGlobal();
						} else if(res.msg == 'category') {
							this.getCategory();
						} else {
							this.getXlc();
						}
      ...
      //     			this.$Message.warning('哎呀！服务器好像出问题了,请稍后重试...')
			...
			getUpdateCustomers(str) {
				//修改费率模式
				fetchPostJson('/nxp-crm/sys/setting/edit',{
					insId:this.rowObj.id,
					way: str
				}).then((res) => {
					if(res.flag != 0) {
            this.$Message.error(res.msg);
			...
			getGlobal() {
				fetchGet('/nxp-crm/sys/subsidy/all/list?insId='+this.rowObj.id).then((res) => {
					if(res.flag != 0) {
            this.$Message.error(res.msg);
					} else {
						for(var i = 0; i < res.data.length; i++) {
							for(var j = 0; j < this.costData.length; j++) {
								if(this.costData[j].pjTypeId == res.data[i].pjTypeId) {
									this.costData[j].rate = res.data[i].rate;
								}
							}
			...
			getCategory() {
				fetchGet('/nxp-crm/sys/subsidy/category/list?insId='+this.rowObj.id).then((res) => {
					if(res.flag != 0) {
            this.$Message.error(res.msg);
					} else {
			...
			getXlc() {
				fetchGet('/nxp-crm/sys/subsidy/xlc/list?insId='+this.rowObj.id).then((res) => {
					if(res.flag != 0) {
            this.$Message.error(res.msg);
					} else {
        ...
        for(var j = 0; j < this.costData.length; j++) {
			...
			okAllModal() {
				this.verify();
				if(this.verifynum == 0) {
					fetchPostJson('/nxp-crm/sys/subsidy/update',{
						insId:this.rowObj.id,
						type: 'all',
          ...
          this.$Message.error("请输入小于100的数字");
			...
			del(catId) {
				fetchPostJson('/nxp-crm/sys/cat/remove/',{
        ...
        for(var j = 0; j < this.costData.length; j++) {
          this.costData[j].rate = 0;
				...
				for(var i = 0; i < this.addClassify.detail.length; i++) {
					this.addClassify.detail[i].rate = 0;
				}
			},
			okCategoryAdd(type) {
				if(type != 'weifenlei') {
            ...
            this.addClassify.insId=this.rowObj.id
						fetchPostJson('/nxp-crm/sys/cat/add',this.addClassify).then((res) => {
							if(res.flag != 0) {
                this.$Message.error(res.msg);
							} else {
            ...
            this.$Message.error("分类名称或管理费填写有误！");
					...
					if(this.addClassify.name != '' && this.addClassify.name != null && this.addClassify.name != undefined && this.verifynum == 0) {
            this.addClassify.insId=this.rowObj.id;
						fetchPostJson('/nxp-crm/sys/cat/edit',this.addClassify).then((res) => {
							if(res.flag != 0) {
                this.$Message.error(res.msg);
							} else {
								this.$Message.success('提交成功!');
			...
			addXlc() {
				...
				fetchPostJson('/nxp-crm/sys/xlc/query',{
					insId:this.rowObj.id,
			...
			chk(id, categoryId) {
				...
				console.log(this.categoryAllArr)
			},
			allSel(checked, Data) {
				console.log(checked);
				if(checked == true) {
			...
			classifyRemove(id) {
				...
				this.$Modal.confirm({
					...
					onOk: () => {
						fetchGet("/nxp-crm/sys/cat/remove/" + id).then((res) => {
							if(res.flag != 0) {
                this.$Message.error(res.msg);
							} else {
								this.getCategory();
			...
			xlcUpdate(name, id) {
				...
				this.modal4 = true;
				console.log(this.xlcData);
				for(var i = 0; i < this.xlcData.length; i++) {
					var istrue = true;
					if(this.xlcData[i].xlcId == id) {
						// for(var j = 0; j < this.xlcData[i].detail.length; j++) {
						// 	if(this.xlcData[i].detail[j].rate == undefined || this.xlcData[i].detail[j].rate == 0 || this.xlcData[i].detail[j].rate == null) {
						// 		istrue = false;
						// 	}
						// }
						if(istrue != false) {
			...
			okXlcInvoice() {
				this.verify('xlc');
				if(this.verifynum == 0) {
					fetchPostJson('/nxp-crm/sys/xlc/update',{
						insId:this.rowObj.id,
						xlcIds: this.xlcIds,
