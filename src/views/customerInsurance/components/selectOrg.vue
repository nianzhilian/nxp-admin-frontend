...
export default {
  ...
  methods:{
    ...
    updateCar(){
      ...
      let name = this.carByData.map(item => item.xlcName)
      fetchPostUrlencoded('/nxp-admin/cos/update',{
        insId:this.rowObj.id,
        insName:this.rowObj.fullName,
        xlcIds:model.join(),
        xlcNames:name.join()
      }).then((res)=>{
        if(res.flag == 0){
          ...
          _this.$Modal.warning({
            ...
            onOk: function() {
              fetchPostUrlencoded('/nxp-admin/cos/update',{
                insId:_this.rowObj.id,
                insName:_this.rowObj.fullName,
                xlcIds:model.join(),
                xlcNames:name.join(),
                force: 'Y'
              }).then((res)=>{
                if(res.flag == 0){
                  tipsModal('更新修理厂成功!',_this.$Modal,'success');
                }else{
                  tipsModal(res.msg,_this.$Modal,'warning');
                }
              })
            }
          });
