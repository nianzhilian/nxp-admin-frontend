...
export function logHttp(source, curName, http) {
  let obj = {
...
// 用户访问日志记录
export function logInfo(obj, http) {
  http('/nxp-admin/frontendLog/log', {
    sourceUri: obj.sourceUri,
    currentUri: obj.currentUri,
    currentName: obj.currentName,
    userBrowser: userClient.userBrowser,
    userOs: userClient.userOs,
    internalIp: userClient.internalIp
  }).then((res) => {
    if (res.flag == 0) {
      console.log(res);
    } else {
...
export function getStorage(store) {
  ...
  if (storageStr) {
    ...
    store.commit('setToken', _storage.data ? _storage.data.token : '');
...
// 判断用户权限
export function userPower(code) {
  let codes = JSON.parse(window.localStorage['user'])['permInfo'];
  let re = false;
  for (let i = 0; i < codes.length; i++) {
    if (codes[i] == code) {
      ...
      if (codes[i]['childNodes']) {
        for (let j = 0; j < codes[i]['childNodes'].length; j++) {
          if (codes[i]['childNodes'][j]['code'] == code) {
            re = true;
            break;
          }
        }
